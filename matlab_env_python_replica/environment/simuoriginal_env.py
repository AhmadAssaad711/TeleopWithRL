"""Gymnasium interface for the nonlinear SimuOriginal plant replica.

This module is the single environment contract used by the Q-learning, DQN,
and policy-gradient implementations. It does not contain notebook-specific
training code; notebooks configure and call the reusable modules in the
algorithm packages.

The wrapper has two layers:

* :class:`SimuOriginalReplicaEnv` exposes the Gymnasium ``reset``/``step``
  API and converts actions, observations, rewards, and diagnostics.
* :mod:`simuoriginal_replica` integrates the underlying 12-state pneumatic
  plant with a fixed-step RK4 solver.

Inputs
------
The constructor accepts environment and experiment settings such as the
force waveform, episode duration, environment-switch time, FE mode, stroke
handling, and action levels. The physical model can be overridden with a
``ParmsOriginal`` instance and a saved-model ``SimuOriginalProfile``. At
runtime, ``reset(seed=..., options=...)`` can override episode settings and
initial conditions, while ``step(action)`` accepts either an integer action
index or a scalar voltage (including a one-element NumPy array).

Outputs
-------
``reset`` returns ``(observation, info)``. ``step`` returns the Gymnasium
5-tuple ``(observation, reward, terminated, truncated, info)``. Observations
are 10-element ``float32`` vectors in this exact order:

``[x_s_centered, x_m_centered, v_s, v_m, P_s1, P_s2, P_m1, P_m2,
mdot_L1, mdot_L2]``.

Each component is normalized by the corresponding ``OBS_SCALE_*`` constant
in :mod:`..config.config`; the positions are centered around ``x_eq`` before
scaling. The ``info`` dictionary contains physical signals, reward-related
diagnostics, termination flags, environment mode, and the applied/requested
voltage. ``render()`` returns the accumulated episode history dictionary.

Timing and reward
-----------------
One RL step advances ``sub_steps = RL_DT / internal_dt`` RK4 substeps. The
reward is the negative weighted sum of normalized tracking error, control
effort, and force/velocity transparency error. Invalid states, volume or
stroke violations, configured tracking-error failures, and the episode step
limit are reported through ``terminated``, ``truncated``, and ``info``.
"""
...
class SimuOriginalReplicaEnv(gym.Env):
    """Gymnasium wrapper around the nonlinear SimuOriginal plant replica.

    The internal physical state has 12 entries in the backend's saved-model
    order ``[Pm1, Pm2, xm_dot, xm, Ps1, Ps2, xs_dot, xs, mL1_dot,
    mL2_dot, x_v, x_v_dot]``. The public RL observation is a derived,
    normalized 10-element vector; callers should use ``info`` or
    ``render()`` when they need the additional physical signals.
    """
    ...
    def __init__(
        ...
        """Create a configured replica environment.

        Parameters
        ----------
        render_mode:
            Gymnasium render selection. ``render()`` returns history data;
            there is no interactive renderer in this wrapper.
        env_mode:
            ``constant_skin`` or ``changing_skin_fat``. The latter applies
            the saved environment-parameter switch at ``env_switch_time``.
        master_input_mode:
            Must be the configured force-driven mode. Position-driven master
            input is not implemented by this replica.
        episode_duration, env_switch_time:
            Episode length and environment-switch time in seconds.
        terminate_on_error:
            Whether a large master/slave position error terminates an episode.
        legacy_baseline_env, reset_position_mode:
            Select the historical origin-centered baseline or the newer
            midpoint-centered RL-safe initialization.
        enforce_stroke_limit, stroke_limit_mode:
            Enable stroke validation and choose ``terminate`` or ``clamp``.
        edge_action_damping_buffer_m, edge_action_min_scale:
            Optionally reduce actions near the physical stroke boundaries.
        action_levels:
            Optional discrete voltage table. Integer actions index this table;
            continuous actions are clipped to its minimum and maximum.
        parms, profile:
            Optional physical parameters and saved SimuOriginal block profile.
        reset_options:
            Default options merged into every later ``reset`` call.
        """
        super().__init__()
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        """Reset the plant and return ``(observation, info)``.

        ``seed`` is passed to Gymnasium's seeding mechanism. ``options`` is a
        dictionary of per-episode overrides; it may contain force settings,
        FE mode, environment parameters, duration/switch time, and an
        ``initial_state`` or ``initial_state_delta``. The returned observation
        is the normalized 10-element vector documented at module level, and
        ``info`` describes the initial physical state and active configuration.
        """
    ...
    def _step_with_voltage(self, u_v: float):
        """Advance one RL interval using the already-resolved voltage.

        The voltage is applied for ``sub_steps`` internal RK4 integrations.
        The method then computes reward and termination state from the updated
        plant, records the transition in the episode history, and returns the
        standard Gymnasium step tuple. It is shared by ``step`` and
        ``step_voltage`` so discrete and continuous callers use identical
        dynamics and reward logic.
        """
        ...
        if not self.invalid_state:
            # The agent sees one transition per RL interval, while the plant
            # is integrated with the finer saved-model time step.
        ...
        # Reward is computed after the complete RL interval, using the same
        # physical signals that are exposed through ``info`` and history.
        pos_error = float(self.state[self.IX_XM] - self.state[self.IX_XS])
    ...
    def step(self, action: int | float | np.ndarray):
        """Apply a discrete index or scalar voltage and return one transition.

        Integer actions index ``action_levels``. A float or one-element array
        is interpreted as a voltage and clipped to the configured voltage
        range. The return value is ``(obs, reward, terminated, truncated,
        info)``.
        """
        return self._step_with_voltage(self._action_to_voltage(action))

    def step_voltage(self, u_v: float):
        """Apply a scalar voltage directly and return a Gymnasium step tuple."""
    ...
    def _get_obs(self) -> np.ndarray:
        """Build the normalized 10-element public observation vector."""
        x_m_centered, x_s_centered = self.get_centered_positions()
        return np.array(
            [
    ...
    def _get_info(self) -> dict:
        """Build diagnostics for the current state and most recent transition."""
        x_m_centered, x_s_centered = self.get_centered_positions()
        human_impedance = force_velocity_impedance(self.F_h, self.state[self.IX_VM])
        environment_impedance = force_velocity_impedance(self.F_e, self.state[self.IX_VS])
        transparency_ratio = force_velocity_transparency_ratio(
    ...
    def discretise_obs(self, obs: np.ndarray) -> tuple[int, ...]:
        """Map the continuous observation to the configured tabular bins."""
        return (
    ...
    def get_state_dims(self) -> tuple[int, ...]:
        """Return the number of bins for each tabular state component."""
        return (
