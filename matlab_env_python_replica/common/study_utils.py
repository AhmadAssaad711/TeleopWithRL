"""Shared result, rollout, metric, and plotting utilities for experiments."""
...
# TensorBoard imports TensorFlow in many installations. Keep it out of module
# import time so notebook setup, ``--help``, and result-only analysis remain
# lightweight; training loads it only when a writer is actually requested.
SummaryWriter = None
...
def require_tensorboard():
    """Import and return TensorBoard's writer only when training needs it."""
    global SummaryWriter, _SUMMARY_WRITER_IMPORT_ATTEMPTED
    if not _SUMMARY_WRITER_IMPORT_ATTEMPTED:
