...
def evaluate_agent(
    ...
    """Run *n_episodes* with greedy policy; return averaged metrics."""
...
def train() -> QLearningAgent:
    ...
    print("=" * 60)
    print(f"  State dims :  {state_dims}  →  {n_states:,} discrete states")
    print(f"  Actions    :  {cfg.N_ACTIONS}")
    print(f"  Q-table    :  {n_states * cfg.N_ACTIONS:,} entries")
    ...
    print(f"  Q-table saved  : results/models/q_table.npy")
    print(f"  Log saved      : results/logs/training_log.npz")
    print(f"  {agent}")
    print("=" * 60)
